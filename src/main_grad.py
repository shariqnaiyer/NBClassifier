...
# python3 src/main_grad.py --train data/train.csv --test data/test.csv --output output/test_grad.csv
# The script src/main_grad.py should receive these command line options:
...
def BOW(sentences, skip_top=20):
    ...
    # Remove the top N most common terms
    common_terms = bag[:skip_top]
    bag = [word for word in bag if word not in common_terms]
...
class NBClassifier:
    def __init__(self, alpha=3):
        ...
        self.likelihoods = {}
        self.bag_size = 0
        self.alpha = alpha  # Lidstone smoothing parameter
    ...
    def train(self, X, y):
        ...
        self.likelihoods = {c: np.ones(self.bag_size) * self.alpha for c in self.priors}
        ...
        for c in y.unique():
            ...
            self.likelihoods[c] = (word_count + self.alpha) / (total_word_count + self.bag_size * self.alpha)
